...
if __name__ == '__main__':
    ...
    while (iiter<500000): 
        ...
        alfa_new = random.normalvariate(4.0, 1)
        beta_new = random.normalvariate(0.83, 0.1)
        gama_new = random.normalvariate(-0.41, 0.1)
        
        try: 
            M_f = M_exp_f(alfa_new, beta_new, gama_new)
            for i in range(N):
               Mv_lum[i] = 10**M_f(logK[i])
        except:
            continue
        ...
        if random.uniform(0, 1.) < ratio:
            ...
            row = [iiter, chi2, alfa, beta, gama]

            myTable.add_row(row)
            
            if iiter%5==0:
                myTable.write('v3seed200.txt', format='ascii.fixed_width',delimiter=' ', bookend=False)
