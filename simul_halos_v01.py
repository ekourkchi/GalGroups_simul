...
################################################################
def readgalList(table, Vls_max=4000, d_min=0, d_max=100):
  ...
  for i in range(N_galaxies):
    ...
    galid = 1000000+i
    if Vls[i] < Vls_max and dist[i]>=d_min and dist[i]<d_max:         
    ...
    for entity in singles:
	    ...
	    for i in range(len(G_list)): # group_indices[0]:
		    ...
		    if ang12 <= 2*r or DDD==True:
		      ...
		      if DDD==True or (ang12 <= 1.01*(180./pi)*atan(1.0*G_list[i][0].R_2t2/d) and abs(entity[0].Vls - G_list[i][0].Vls) < 2.5*sig_p):
			  d1 = G_list[i][0].mDist
			  e1 = d1 * 0.05
			  d2 = entity[0].dist
			  e2 = d2 * 0.05
...
if __name__ == '__main__':
  ...
  galList, G_list = readgalList(table, Vls_max=4000, d_min=0, d_max=100)
...
#####################################################################
  ...
  groupWrite('halo.0.100.v01.group', G_list, galList)
