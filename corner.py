...
def hist2d(x, y, bins=20, range=None, weights=None, levels=None, smooth=None,
    ...
    max_H = np.max(H)
    x_max = X[np.where(H==max_H)[0]]
    y_max = Y[np.where(H==max_H)[1]]
    print(x_max, y_max)
    #ax.plot(x_max, y_max, "s", color='green')
    ...
    if True: #(plot_contours or plot_density) and not no_fill_contours:
        ax.contourf(X2, Y2, H2.T, [V.min(), H.max()])

    if False: # plot_contours and fill_contours:
    ...
    # contour fills.
        ...
        cmap = cm.PRGn
        #ax.contourf(X, Y, H, levels, cmap=cm.get_cmap(cmap, len(levels) - 1))
