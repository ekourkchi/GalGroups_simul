...
class GroupBrowser(object):
	...
	i = 0 
		...
		Y[q] = y
	      ...
	      line, = plt.plot(X,Y, ':', markersize = 1, color='red', picker=False, visible=False)
	      Vls_list = []
	      Vls_list.append([Vls[i],sigmaP_lum[i], mDist[i]])
...
class PointBrowser(object):
    ...
    def onpick(self, event):
	  ...
	  myTable = Table()
	  myTable.add_column(Column(data=self.id_gal[ind], name='ID'))
	  ...
	  #myTable.add_column(Column(data=self.MagK[ind],  name='MagK'))
...
def table_deliver(filee):
  ...
  out_table = {'ID':id, 'flag':flag, 'ra':ra, 'dec':dec, 'Ks':Ks, 'Vls':Vls,  \
               'R2t_lum':R_2t2, 'nest':nest, 'dist':dist, \
		'mDist':mDist, 'sigmaP_lum':sigmaP_lum}
  return out_table
#################################################
...
# **************************************
def xyz_list(alpha_list, delta_list, Vls_list, dist_list, mDist_list, flag_list):
...
class VelDist:
    ...
    def show_histogram(self, velocities, replot=False):
	  ...
	  self.ax.yaxis.set_minor_locator(MultipleLocator(1))
        ...
        if dist != 0:
          self.ax.annotate(r"$d$"+":  "+'{:.2f}'.format(dist)+r' Mpc', (0.15,0.5), xycoords='figure fraction')
        else:
...
class Plot_page(tk.Frame):
  ...
  def show_plot(self):
	...
	table_gal = {'ID':id_gal, 'flag':flag_gal, 'ra':ra_gal, 'dec':dec_gal, 'Ks':Ks_gal, 'Vls':Vls_gal,  \
               'R2t_lum':R_2t2_gal, 'nest':nest_gal, 'dist':dist_gal,   \
		 'mDist':mDist_gal, 'sigmaP_lum':sigmaP_lum_gal}
	...
	table_all = {'ID':id, 'flag':flag, 'ra':ra, 'dec':dec, 'Ks':Ks, 'Vls':Vls,  \
               'R2t_lum':R_2t2, 'nest':nest, 'dist':dist,  \
		 'mDist':mDist, 'sigmaP_lum':sigmaP_lum}
	#########  
